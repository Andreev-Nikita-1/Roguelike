...
public class SomeLightingCodeFromInternet {
    ...
    private Function<Coord, Boolean> _blocksLight;
    private Function<Coord, Integer> GetDistance;
    private Consumer<Coord> _setVisible;
    ...
    /// </param>
    public SomeLightingCodeFromInternet(Function<Coord, Boolean> blocksLight, Consumer<Coord> setVisible, Function<Coord, Integer> getDistance) {
        _blocksLight = blocksLight;
        GetDistance = getDistance;
        _setVisible = setVisible;
    }

    public void Compute(Coord origin, int rangeLimit) {
        _setVisible.accept(origin);
        for (int octant = 0; octant < 8; octant++)
            Compute(octant, origin, rangeLimit, 1, new Slope(1, 1), new Slope(0, 1));
    }

    private class Slope // represents the slope Y/X as a rational number
    {
        public int X, Y;
    ...
    void Compute(int octant, Coord origin, int rangeLimit, int x, Slope top, Slope bottom) {
        ...
        for (; x <= (int) rangeLimit; x++) // (x <= (int)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
        {
            ...
            {
                if (rangeLimit < 0 || GetDistance.apply(new Coord((int) x, (int) y)) <= rangeLimit) // skip the tile if it's out of visual range
                {
    ...
    boolean BlocksLight(int x, int y, int octant, Coord origin) {
        ...
        return _blocksLight.apply(new Coord((int) nx, (int) ny));
    }

    void SetVisible(int x, int y, int octant, Coord origin) {
        int nx = origin.x, ny = origin.y;
        switch (octant) {
            case 0:
                nx += x;
        ...
        _setVisible.accept(new Coord((int) nx, (int) ny));
    }
